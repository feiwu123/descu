...
export const useTime = () => {
	...
	const updateTime = () => {
		...
		nowTime.value = `${year.value}年${month.value}月${day.value}`;
		sfmTime.value = `${hour.value}:${minute.value}:${second.value}`;
	};

	updateTime();

	return { year, month, day, hour, minute, second, week, nowTime,sfmTime };
};