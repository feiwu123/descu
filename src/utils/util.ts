 ...
 * 返回一个新的数组 含有children
 * */
export function cateTree(array?: []) {
	let sortArr = array;
	sortArr.forEach((item, index) => {
		sortArr[index] = { ...item, label: item.cat_name, value: item.cat_id, children: [] };
	});
	let parentArr: [] = sortArr.filter(function (item) {
		return item.parent_id == "0";
	});

	function sortCatArr(currentArr, desArr) {
		currentArr.forEach((curr, i) => {
			if (curr.has_children != "0") {
				let cat_id = currentArr[i].cat_id;
				let findArr = desArr.filter(item => {
					return item.parent_id == cat_id;
				});
				currentArr[i].children = findArr;
				sortCatArr(findArr, desArr);
			}
		});
	}
	sortCatArr(parentArr, sortArr);
	return parentArr;
}
